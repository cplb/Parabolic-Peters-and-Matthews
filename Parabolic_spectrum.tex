...
\begin{equation}
\omega_1^2 = \frac{G(M_1 + M_2)(1 - e)^3}{r\sub{p}^3} = (1 - e)^3\omega\sub{c}^2,
...
\begin{figure}
\includegraphics[width=75mm]{Fig_ell.eps}
...
The PM approach assumes Keplerian orbits in flat spacetime. This should be a valid approximation in the weak-field regime far from a massive body. To find the limit of this approximation, we can compare the PM results with those from more accurate techniques. Energy spectra for parabolic orbits do not seem to be available in the literature yet, so we will make do with the total energy fluxes calculated by Martel~\cite{Martel2004a}, who uses time-domain black hole perturbation theory for a Schwarzschild black hole of mass $M$. \Figref{Ratio} shows the ratio of the two energies as a function of periapse distance. As expected the PM result is more accurate for larger periapses. The agreement worsens as the periapsis decreases. At $r\sub{p} = 4 M$, corresponding to the radius of the innermost stable parabolic orbit (ISPO), the energy flux calculated by Martel diverges, so the ratio tends to zero. This divergence is because in Schwarzschild (or Kerr) spacetime a parabolic orbit may have a zoom-whirl structure where it undergoes a number of near circular rotations (whirls) about the black hole. As the radius of the ISPO is approached, the number of whirls tends to infinity (in the absence of radiation reaction), so an infinite amount of energy is radiated.
\begin{figure}
\includegraphics[width=75mm]{Fig_Ratio.eps}
\caption{Ratio of the total energy radiated as calculated using the Peters and Mathews~\cite{Peters1963} approach to that calculated by Martel~\cite{Martel2004a} using black hole perturbation theory (solid line) versus periapse radius $r\sub{p}$. The latter approach should give more accurate results. Also shown is the reciprocal of the number of rotations $1/N$ (dashed line). The Keplerian limit corresponds to $N = 1$.\label{fig:Ratio}}
\end{figure}
...
\end{equation}
the periapsis is the larger root $r\sub{p} > r_1$. This equation implicitly gives $L_z$ as a function of $r_p$. The integral may be rewritten as
\begin{equation}
...
\begin{figure}
\includegraphics[width=75mm]{Fig_N_peri.eps}
\caption{Periapse radius corresponding to $N = 1.1$ as a function of spin parameter $a$ (solid line). The curve may be approximated by a straight line $r\sub{p} = -3.91 a + 17.36M$ (dashed line).\label{fig:N_peri}}
\end{figure}
...
Considering bursts from the Galactic centre, orbits with periapses of $r\sub{p} \lesssim 120 M$ could generate bursts that would be detectable with LISA~\cite{Rubbo2006, Hopman2007}. It is therefore likely that any such burst that was detected would be in the regime of validity of the Peters and Mathews approach, $r\sub{p} \gtrsim 20 M$ for equatorial orbits. The results described in this note will therefore have application in that context, and it should be possible to explore the majority of parameter space using this approximation. The most interesting orbits, those which come deep within the strong-field region of the black hole's spacetime, will be beyond the range of validity of this approximation, but these represent a small subset of all plausible events.

This result may also be applicable for studying parabolic encounters between stellar mass black holes; these may occur in densely populated environments such as globular clusters~\cite{Kocsis2006} or the Galactic centre~\cite{O'Leary2009}. Bursts from these encounters should be detectable with near-future ground-based detectors, such as the Advanced Laser Interferometric Gravitational-Wave Observatory~\cite{Kocsis2006, O'Leary2009}.
